...
需求确认、技术确认、测试准入通过和测试通过分别创建平台检查点。用户确认需求后，平台先保持在 `requirements_pending_confirmation`，在同一 session 中根据完整需求对话生成 `docs/requirements/Rxxx-*/` 正式文档；文档通过编号与存在性校验并提交需求检查点后，系统才切换到 `technical_design` 并自动启动技术设计轮次。生成或校验失败时停留在需求阶段等待用户重试。
