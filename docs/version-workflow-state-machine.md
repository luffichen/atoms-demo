...
4. 未单独回答的事项沿用需求文档中已经记录的推荐默认方案。
5. 平台校验当前 `Rxxx-*` 需求包的存在性与结构，创建需求确认内部检查点。
6. 校验通过后直接进入技术设计并自动启动技术设计任务。
7. 若编号需求包尚未完整落盘，只创建一次原地补全轮次；已有目录不得重复生成。

### 3.3 需求待确认

该阶段只兼容升级前已经进入“需求待确认”的工作项。用户点击“重新执行”时，平台直接校验已有 `Rxxx-*` 需求包、创建检查点并进入技术设计；不会再次生成一套需求文档，也不会按全文关键词拦截历史“待确认”措辞。
